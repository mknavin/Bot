...
import {TransDocComponent} from './trans.docs';
 ...
 */
@Component ({
  directives: [PayloadComponent, TransDocComponent],
  ...
  <div id='parent' class='parentDiv' (window:resize)='onResize($event)'>
      <div id='scrollingChat'>
        <div id='segments' *ngFor='#segment of segments'>
          <div [class]='segment.isUser() ? "from-user" :
            ...
            <div *ngIf='!segment.isUser() && segment.getTrans().length>0 && segment !== segments[0]'>
   ...
   <tr>
    ...
    <th width="20%">Amount</th>
     ...
     <trans-doc *ngFor='#doc of segment.getTrans()' [doc]='doc'></trans-doc>
   </div>
            </div>
          </div>
          <div class='clear'></div>
  ...
  private callConversationService (chatColumn, payload) {
    ...
    this._dialogService.message (this.workspace_id, payload).subscribe (
      data1 => {
        ...
        if (data1) {
          if (data1.error) {
            ...
            } else if (data1.output.transPayload && data1.output.transPayload.length > 0) {
             trans = data1.output.transPayload;
             responseText = this.langData.Great;
        ...
        this.segments.push (new DialogResponse (responseText, false, trans, data1));
      ...
      error => {
        ...
        this.segments.push (new DialogResponse (serviceDownMsg, false, trans, this.langData.NResponse));
        chatColumn.classList.remove ('loading');
      });
  }
}
